  ...
  setGround(): void {
    if (this.scene) {
      ...
      const floorMat = new THREE.MeshStandardMaterial({
        roughness: 0.8,
        color: 0xffffff,
        metalness: 0.2, //接近金属成都
        bumpScale: 0.0005
      });
      // 加载纹理，给材质上纹理贴图
      this.textureLoader.load("/assets/imgs/hardwood2_diffuse.jpg", map => {
        // 设置纹理属性

        map.wrapS = THREE.RepeatWrapping; //水平垂直重复方式
        map.wrapT = THREE.RepeatWrapping;
        map.anisotropy = 4; //默认1，可以理解为清晰度
        map.repeat.set(10, 24);
        map.colorSpace = THREE.SRGBColorSpace;
        floorMat.map = map; //贴图
        floorMat.needsUpdate = true;
      });
      this.textureLoader.load("/assets/imgs/hardwood2_bump.jpg", map => {
        map.wrapS = THREE.RepeatWrapping;
        map.wrapT = THREE.RepeatWrapping;
        map.anisotropy = 4;
        map.repeat.set(10, 24);
        floorMat.bumpMap = map; //凹凸贴图的纹理,凹凸实际上不会影响对象的几何形状，只影响光照
        floorMat.needsUpdate = true;
      });
      this.textureLoader.load("/assets/imgs/hardwood2_roughness.jpg", map => {
        map.wrapS = THREE.RepeatWrapping;
        map.wrapT = THREE.RepeatWrapping;
        map.anisotropy = 4;
        map.repeat.set(10, 24);
        floorMat.roughnessMap = map; //改变材质的粗糙度
        floorMat.needsUpdate = true;
      });
