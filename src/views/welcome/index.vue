...
onMounted(() => {
  ...
  bigEye.addEventListener("animationend", () => {
    new Promise(res => {
      clearInterval(rotTimer);
      rotTimer = setInterval(() => {
        getEyeballChart();
...
function focusOnMouse(e) {
  ...
  // 原点，即bigEye中心位置，页面中心
  const origin = [clientWidth / 2, clientHeight / 2];
  // 鼠标坐标
  const mouseCoords = [e.clientX - origin[0], origin[1] - e.clientY];
  const eyeXDeg = (mouseCoords[1] / clientHeight) * 80;
  const eyeYDeg = (mouseCoords[0] / clientWidth) * 60;
